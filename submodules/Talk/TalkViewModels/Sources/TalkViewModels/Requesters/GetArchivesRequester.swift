...
@MainActor
public class GetArchivesRequester {
    ...
    public func getCalculated(_ req: ThreadsRequest, withCache: Bool, queueable: Bool = false, myId: Int, navSelectedId: Int?) async throws -> [CalculatedConversation] {
        let conversations = try await get(req, withCache: withCache, queueable: queueable)
        return await calculate(conversations, myId, navSelectedId)
    }
    ...
    private func calculate(_ conversations: [Conversation], _ myId: Int, _ navSelectedId: Int?) async -> [CalculatedConversation] {
