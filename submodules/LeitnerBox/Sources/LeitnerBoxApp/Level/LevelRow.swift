    ...
    private var progressViewAndCount: some View {
        VStack {
            ...
            ProgressView(
                ...
                total: max(Float(levelRowData.totalCountInsideLevel), 1)
            )
    ...
    struct Preview: View {
        ...
        var body: some View {
            LevelRow(levelRowData: .init(leitnerId: 1, level: Level(), favCount: 25, reviewableCount: 50, totalCountInsideLevel: 100),
                     container: LeitnerBoxObjectsContainer(context: Preview.context, leitner: Preview.leitner, leitnerVM: leitnerViewModel))
        }
    }
