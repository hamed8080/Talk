...
final class LeitnerViewModel: ObservableObject {
    ...
    @Published var selectedObjectContainer: LeitnerBoxObjectsContainer?
    ...
    init(viewContext: NSManagedObjectContextProtocol) {
        ...
        NavigationViewModel.shared.$paths.sink { [weak self] newValue in
            guard let self = self else { return }
            if newValue.isEmpty {
                selectedObjectContainer = nil
                selectedLeitner = nil
                NavigationViewModel.shared.columnVisibility = .doubleColumn
            }
    ...
    func setLeithner(_ leitner: Leitner) {
        ...
        selectedObjectContainer = LeitnerBoxObjectsContainer(context: viewContext as! NSManagedObjectContext, leitner: leitner, leitnerVM: self)
        NavigationViewModel.shared.append(leitner: leitner)
    }
}
